'use strict';

const passengerTransportation = {
    "name": 'COMPANY',
    "phone": '+8 (800) 123-45-67',
    "mail": 'sales@log.com',
    "language": ['en', 'ua', 'ru'],
    "title": {
        "en":  'Passenger Transportation',
        "ua":  'Пассажирські Перевезення',
        "ru":  'Пассажирские Перевозки',
    },
    "mainMenu": {
        "en": [
            'About US',
            'Services',           
            'Contacts',
        ],
        "ua": [             
            'Про Нас',
            'Сервіси',
            'Контакти',
        ],
        "ru": [            
            'О Нас',
            'Сервисы',
            'Контакты',
        ],
    },
    "articleHeader": {
        "en": [
            'TEXT CONTENT COMPANY',
            'Subtitle text content dlkkldvbd dbdgvbgd dvbndjkl',
        ],
        "ua": [
           'ТЕКСТ КОНТЕНТ КОМПАНІЇ',
            'Сабтайтл текст контент на українській мові компанії',
        ],
        "ru": [
           'ТЕКСТ КОНТЕНТ КОМПАНИИ',
            'Сабтайтл текст контент на русском языке компании',
        ],
    },
    "aboutUs": {
        "en": [
            'ABOUT US',
            'Lorem ipsum dolor sit amet consectetur adipisicing elit. Praesentium reiciendis inventore alias adipisci omnis ullam, commodi reprehenderit. Neque sequi deserunt perspiciatis dolor rerum est saepe cumque, quos et, eaque itaque?',
            'more',
        ],
        "ua": [
            'ПРО НАС',
            'Але щоб ви зрозуміли, звідки виникає це хибне уявлення людей, цуратись насолоди і вихваляти страждання, я розкрию перед вами всю картину і роз’ясню, що саме говорив цей чоловік, який відкрив істину, якого я б назвав зодчим щасливого життя.',
            'докладніше',
        ],
        "ru": [
            'О НАС',
            'Сайт рыбатекст поможет дизайнеру, верстальщику, вебмастеру сгенерировать несколько абзацев более менее осмысленного текста рыбы на русском языке, а начинающему оратору отточить навык публичных выступлений в домашних условиях. При создании генератора мы использовали небезизвестный универсальный код речей.',
            'подробнее',
        ],
    },
    "services": {
        "en": {
            "mainTitle": ['SERVICES'],
            "card": [
                {   
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Card',
                    "description": 'Dinos were great!',
                    "btn": 'more',
                },
            ]
        },
        "ua": {
            "mainTitle": ['СЕРВІСИ'],
            "card": [
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Картка',
                    "description": 'Опис тексту відповідної картки!',
                    "btn": 'докладніше',
                },
            ],
        },
        "ru": {
            "mainTitle": ['СЕРВИСЫ'],
            "card": [
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
                {
                    "imgSrc": './assets/images/Rectangle-1.png',
                    "title": 'Карточка',
                    "description": 'Описание какой - то карточки!',
                    "btn": 'подробнее',
                },
            ]
        },
    },
    "contactsTitle": {
        "en": ['CONTACTS'],
        "ua": ['КОНТАКТИ'],
        "ru": ['КОНТАКТЫ'],
    },
    "contacts": {
        "facebook": {
            "title": 'Facebook',
            "link": './',
        },
        "viber": {
            "title": 'Viber',
            "link": './',
        },
        "telegramm": {
            "title": 'Telegramm',
            "link": './',
        },
        "whatsapp": {
            "title": 'WhatsApp',
            "link": './'
        },
    },
};


